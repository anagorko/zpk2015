//wszystkie informacje na temat konkretnych funkcji biblioteki allegro (przeznaczenie, działanie, składnia) czerpałam ze strony http://cpp0x.pl/dokumentacja/Allegro-4-4/10
...
while(warunek_menu){
    ...
    while( (double)(clock()-ticks)/CLOCKS_PER_SEC<0.6){     //wykonuj petle dopoki czas nie przekroczy 0.6s
     ...
     if(key[KEY_ENTER] || key[KEY_SPACE])   {               //jesli enter lub spacja
...
int game(BITMAP *bufor){
    ...
    BITMAP * mapa=NULL;                     //wskaźnik na bitmape mapy
    BITMAP * waz=NULL;                      //wskaźnik na bitmape weza
    BITMAP * pokarm=NULL;                    //wskaźnik na bitmape pokarmu
...
std::vector <wsp> ogony;                    //vector wspolrzednych, w ktorym przechowujemy wspolrzedne czesci ciala weza
int snake_x=200, snake_y=200;               //pozycja poczatkowa snake'a
int snake_pos=0;                            //kierunek w ktory jest skierowany snake
bool byl_klawisz=false;                     //flaga zapobiegajaca kilkukrotnym wcisnieciom klawisza
...
//powiekszanie weza - skorzystalam z podpowiedzi na forum http://cpp0x.pl/forum/temat/?id=10181
//zeby w kodzie dodac nowy fragment weza nie na koncu, ale zaraz za glowa, bo pozycja glowy jest latwa do odczytania
    if(snake_x==pozywka.x && snake_y==pozywka.y){       //jesli glowa weza jest na polu z pokarmem
        ...
        }                                                    //podpowiedz jak poruszac calym wezem: http://forum.pcformat.pl/C-Allegro-5-Snake-i-czesci-ciala-t
...
int o_grze(BITMAP *bufor){
     ...
     textprintf_ex( bufor, font, 75, 160, makecol( 200, 200, 200 ), -1, "Za każdy zebrany pokarm dostaniesz okreslona liczbe punktow obliczaną na podstawie obecnego poziomu oraz tego jak szybko Ci sie to udalo!" );
     textprintf_ex( bufor, font, 75, 200, makecol( 200, 200, 200 ), -1, "Aby powrocic nacisnij ESC" );
     blit( bufor, screen, 0, 0, 0, 0, 1200, 800 );
