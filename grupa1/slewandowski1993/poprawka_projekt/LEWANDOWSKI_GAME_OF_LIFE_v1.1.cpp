...
class Plansza
{
	...
	void Obraz(int x, int y, int z, int k);
...
void Plansza::Obraz(int x, int y, int z, int k)
{
    al_clear_to_color(al_map_rgb(z, 0, 0));
	for(int j=0;j<h;j++)
		{
		for(int i=0;i<w;i++)
		{
				if(Tablica[i][j]!=0)
				al_draw_pixel( k*i, k*j, al_map_rgb(y,255,255));
		}
...
int main(int, char**)
{
    ...
    Wczytaj_mape1:
        ...
        cin>>odKtorego;
        cout << "Ile milisekund ma trwac jedna plansza?";
        cin>>ileSek;
        cout << "Jakiego koloru ma byc punkt wg. skali ALLEGRO_COLOR?";
        cin>>jakiKolor;
        cout << "Jakiego koloru ma byc plansza wg. skali ALLEGRO_COLOR?";
        cin>>jakaPlansza;
        cout << "Jaka ma byc odleglosc pomiedzy pikselami od 1 do 10?";
        cin>>jakaOdl;
        ...
        for (int i=0; i<=ileOkresow; i++) {
            if(i<odKtorego)
                ...
                p->PrzeliczPlansze();
                p->Obraz(ileSek/1000,jakiKolor, jakaPlansza, jakaOdl);
            }
        };
        goto Symulacja;
    }
    ...
    Gra_uzytkownika_wybor:
        ...
        al_draw_text(title2, al_map_rgb(200, 200, 200), w / 2 , 120, ALLEGRO_ALIGN_CENTRE, "WCZYTAJ PLANSZE");
        al_draw_text(title, al_map_rgb(200, 200, 200), w / 2 , 250, ALLEGRO_ALIGN_CENTRE, "PRZYKŁADOWA GRA - GÓRA");
        al_draw_text(title, al_map_rgb(200, 200, 200), w / 2 , 300, ALLEGRO_ALIGN_CENTRE, "WŁASNA GRA - DÓŁ");
        al_draw_text(title, al_map_rgb(200, 200, 200), w / 2 , 350, ALLEGRO_ALIGN_CENTRE, "POWRÓT - LEWO");
        al_draw_text(title, al_map_rgb(200, 200, 200), w / 2 , 450, ALLEGRO_ALIGN_CENTRE, "ESCAPE - WYJŚCIE");
    ...
    Przykladowa_gra:
        ...
        al_draw_text(title2, al_map_rgb(200, 200, 200), w / 2 , 120, ALLEGRO_ALIGN_CENTRE, "PODAJ LICZBĘ KOMÓREK ");
        al_draw_text(title2, al_map_rgb(200, 200, 200), w / 2 , 180, ALLEGRO_ALIGN_CENTRE, "OD 5 DO 7");
    ...
    while (true) {
        ...
        p.DodajZycie(124,124);
        p.DodajZycie(125,124);
        p.DodajZycie(125,125);
        p.DodajZycie(125,126);
        p.DodajZycie(124,123);

        for (int i=0; i<=100; i++) {
            p.PrzeliczPlansze();
            p.Obraz(1000/1000, 255, 5, 5);
            };
    ...
    while (true) {
        ...
        p.DodajZycie(125,126);
        p.DodajZycie(124,123);
        p.DodajZycie(125,127);

        for (int i=0; i<=100; i++) {
            p.PrzeliczPlansze();
            p.Obraz(100/1000, 255, 5, 5);
            };
