...
//element tablicy to element w dlugim wierszu
	switch(mPlansza[MrowaPosition.first + MrowaPosition.second * MAP_Wys])
	{
		case czarny:
			mMrowa.wPrawo();
			mPlansza[MrowaPosition.first + MrowaPosition.second * MAP_Wys] = bialy;
			break;
		case bialy:
			mMrowa.wLewo();
			mPlansza[MrowaPosition.first + MrowaPosition.second * MAP_Wys] = czarny;
			break;
	}
...
//Poni¿sze polecenia pochodz¹ z dokumentacji biblioteki allegro co do transformacji z "oka kamery"
	...
	float tileBitmapSzer = TILE_Szer - 2;	// 2 - szerokoœæ obramowania okna
	float tileBitmapWys = TILE_Wys - 2;	// 2 - szerokoœæ obramowania okna
	...
	for(int y = 0; y < MAP_Wys; y++)
	{
		...
		{
			int tilesBitmapX = -1; //krañce planszy
			int tilesBitmapY = 1;
...
//G£ÓWNY program
ALLEGRO_BITMAP * initmojaBitmapa(ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *mojaBitmapa, int tileSzer, int tileWys);

int main(void)
{
//wskaŸniki obiektów na null
	ALLEGRO_DISPLAY *display = nullptr;
	ALLEGRO_BITMAP *mojaBitmapa = nullptr;
	ALLEGRO_TIMER *timer = nullptr;
	ALLEGRO_EVENT_QUEUE *queue = nullptr;
	ALLEGRO_FONT *font = nullptr;
	bool redraw = true; //ekran sie przerysowuje ciagle
	int mouse = 0; 	//klawisz myszy (0 to zaden nie wcisniety)
	long licznik = 0; //podaje liczbe iteracji

	//Inicjalizacja biblioteki allegro i dodatków, np. obs³ugi myszy i klawiatury
	al_init();
	al_init_image_addon();
	...
	Plansza map(display);
	mojaBitmapa = initmojaBitmapa(display, mojaBitmapa, map.getTileSzer(), map.getTileWys());

	font = al_create_builtin_font();

//Ustawienie licznika czasu i kolejki zdarzeñ
	...
	while(true)
	{
		...
		//Klawisze steruj¹ce: ESC zamyka program, SPACJA pauzuje
...
// Bitmapa jest zlozeniem czterech figur: czarnego kwadratu, bialego kwadratu, trojkata czerwonego skierowanego w gore, czerwonego trojkata skierowanego w prawo
	...
	ALLEGRO_COLOR czarny = al_map_rgb(68, 68, 68);
	ALLEGRO_COLOR czerwony = al_map_rgb(255, 0, 0);

	mojaBitmapa = al_create_bitmap(4 * tileSzer, tileWys);
	al_set_target_bitmap(mojaBitmapa);
	al_clear_to_color(al_map_rgba(0, 0, 0, 0));

	al_draw_filled_rectangle(0, 0, tileSzer, tileWys, czarny);
	al_draw_filled_rectangle(tileSzer, 0, 2 * tileSzer, tileWys, bialy);
	al_draw_filled_triangle(2 * tileSzer + tileSzer*0.5f, tileWys* 0.125f,
							2 * tileSzer + tileSzer*0.125f, tileWys* 0.875f,
							2 * tileSzer + tileSzer*0.875f, tileWys* 0.875f, czerwony);	// UP and DOWN
	al_rysuj_filled_triangle(3 * tileSzer + tileSzer*0.125f, tileWys* 0.125f,
							3 * tileSzer + tileSzer*0.125f, tileWys* 0.875f,
							3 * tileSzer + tileSzer*0.875f, tileWys* 0.5f, czerwony);	// RIGHT and LEFT

//Ustawienie bufera ¿eby nie miga³ ekran
