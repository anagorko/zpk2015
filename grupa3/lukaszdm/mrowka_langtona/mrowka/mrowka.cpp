...
void mrowa::pomaluj(vector <pole> &NieBiale, int kolory)
{
     ...
     for(int i = 0; i < NieBiale.size(); i++)
    {
        if(pozX == NieBiale[i].getX() && pozY == NieBiale[i].getY())
         ...
         pole v(pozX, pozY, rozmiar); //je˝eli przelecia∏a ca∏a p´tla, a funkcja nie zosta∏a zakonczona, znaczy ze mrowka stala na bialym polu, trzeba je dodac jako czarne (domyslny kolor)do wektora NieBiale
...
int mrowa::czySieZmiesci(vector <pole> &NieBiale)
{
 ...
 skret(NieBiale); //skreca mrówk´ "w gotowosci" do nastepnego ruchu
