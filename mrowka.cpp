...
vector <int> skrajne_pozycje(vector <pole> &NieBiale) // argumentem funkcji jest wektor zawierajˆcy niebia¸e pola. funkcja zwraca wektor z 4roma wartosciami- skrajnymi wspolrzednymi pol nalezacych do argumentu
{
    vector <int> skrajne;
    skrajne.push_back(1000); //Najmniejszy x
    skrajne.push_back(1000); //Najmniejszy y
    skrajne.push_back(0); //Najwiekszy x
    skrajne.push_back(0); //Najwiekszy y
    for(int i = 0; i < NieBiale.size(); ++i)
    {
...
void zmniejszMrowki(vector<mrowa> &Mrowy, vector <pole> &NieBiale, int rozmiar)
{
	...
	int wys = 600;
	vector <int> Pozycje;
	Pozycje = skrajne_pozycje(NieBiale);
	int maxY = 0.5 * wys;//Pozycje[3];
	int maxX = 0.5 * szer;//Pozycje[2];
	int roznicaX = 0;
	int roznicaY = 0;
	int roznica = 10;
	if (rozmiar <= 10)
